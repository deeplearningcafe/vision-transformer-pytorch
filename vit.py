...
class LinearEmbedding(nn.Module):
    def __init__(self, conf: omegaconf.DictConfig) -> None:
        ...
        self.positional_embeds = nn.Parameter(torch.zeros((self.seq_len + 1, self.hidden_dim)))
...
class MultiHeadSelfAttention(nn.Module):
    def __init__(self, conf: omegaconf.DictConfig) -> None:
        ...
        self.projection = nn.Linear(in_features=self.hidden_dim, out_features=self.hidden_dim, bias=False)
        self.dropout = nn.Dropout(p=conf.vit.dropout)
